...
def evaluate_sequential(args, runner):
    ...
    if args.runner in ("episode_xp", "episode"):
        stats, returns = res
        print("test_dead_enemies_mean: %.4f" % (stats["dead_enemies"] / stats['n_episodes']))
        print("test_return_mean: %.4f" % (sum(returns) / stats['n_episodes']))
        print("test_ep_length_mean: %.4f" % (stats["ep_length"] / stats['n_episodes']))
        print("test_n_episodes: %d" % (stats['n_episodes']))
...
def run_sequential(args, logger):
    ...
    # DIY: extend the obs vector with a one-hot encoding of the data collector
    # 注意：这里的obs_vshape指每个agent的观测向量维度

    obs_id_len = getattr(args, "obs_id_len", None)
    if obs_id_len != None:
        print("using one-hot encoding in obs for data collector id".upper())
        obs_vshape = env_info["obs_shape"] + obs_id_len
        print("obs_vshape: %d" % obs_vshape)
    else:
        obs_vshape = env_info["obs_shape"]

    scheme = {
        "state": {"vshape": env_info["state_shape"]},
        "obs": {"vshape": obs_vshape, "group": "agents"},
    ...
    if args.runner == "episode_xp": # DIY: 仅临时实现cross-play
        # 注意：BC策略和QMIX训练的专家策略模型格式不同，要用不同的learner
        learner = le_REGISTRY[args.ego_learner](mac, buffer.scheme, logger, args)
        if args.use_cuda:
            learner.cuda()
    ...
    if args.runner == "episode_xp" and args.checkpoint_path2 != "":
        timesteps = []
        timestep_to_load = 0

        if not os.path.isdir(args.checkpoint_path2):
            logger.console_logger.info("Checkpoint directiory {} doesn't exist".format(args.checkpoint_path2))
            return

        # Go through all files in args.checkpoint_path
        for name in os.listdir(args.checkpoint_path2):
            full_name = os.path.join(args.checkpoint_path2, name)
            # Check if they are dirs the names of which are numbers
            if os.path.isdir(full_name) and name.isdigit():
                timesteps.append(int(name))
        ...
        model_path = os.path.join(args.checkpoint_path2, str(timestep_to_load))

        logger.console_logger.info("Loading model from {}".format(model_path))
        learner2.load_models(model_path)
    

    if (args.runner == "episode_xp" and args.checkpoint_path != "" and args.checkpoint_path2 != "") \
        or (args.runner != "episode_xp" and args.checkpoint_path != ""):
